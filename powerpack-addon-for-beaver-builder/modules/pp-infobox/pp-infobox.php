...
class PPInfoBoxModule extends FLBuilderModule {
	...
	public function render_title() {
		...
		$text        = in_array( $link_type, $valid_types ) ? sprintf(
			...
			esc_url( do_shortcode( $settings->link ) ),
			esc_attr( $settings->link_target ),
			$nofollow,
			$settings->title
		) : $settings->title;
		$title = sprintf( '<%1$s class="pp-infobox-title">%2$s</%1$s>', esc_attr( $settings->title_tag ), $text );
		?>
	...
	public function render_title_prefix() {
		...
		$prefix_tag = ( isset( $settings->title_prefix_tag ) ) ? esc_attr( $settings->title_prefix_tag ) : 'span';
		?>
	...
	{
		$settings 		= $this->settings;
		$button_class 	= ( 'button' == $settings->pp_infobox_link_type && '' != $settings->link_css_class ) ? ' ' . esc_attr( $settings->link_css_class ) : '';
		$nofollow		= ( isset( $settings->link_nofollow ) && 'yes' == $settings->link_nofollow ) ? ' rel="nofollow"' : '';
		...
		if ( 'button' === $settings->pp_infobox_link_type || 'button+title' === $settings->pp_infobox_link_type || $force_render ) {
			...
			<div class="pp-infobox-button pp-button-wrap">
				<<?php echo $link_tag; ?> class="pp-more-link pp-button<?php echo $button_class; ?>" href="<?php echo esc_url( do_shortcode( $settings->link ) ); ?>" role="button" target="<?php echo esc_attr( $settings->link_target ); ?>"<?php echo $nofollow; ?>>
		...
		if ( 'read_more' === $settings->pp_infobox_link_type && ! $force_render ) {
			...
			<div class="pp-infobox-button">
				<p>
					<a class="pp-more-link<?php echo $button_class; ?>" href="<?php echo esc_url( do_shortcode( $settings->link ) ); ?>" role="button" target="<?php echo esc_attr( $settings->link_target ); ?>"<?php echo $nofollow; ?>>
						<?php echo $settings->pp_infobox_read_more_text; ?>
					</a>
				</p>
			</div>
					...
					'title_prefix_margin'      => array(
						...
						'options'           => array(
							'top'               => array(
								...
								'preview'           => array(
									'selector'          => '.pp-infobox .pp-infobox-title-prefix',
									'property'          => 'margin-top',
									'unit'              => 'px'
								),
							),
